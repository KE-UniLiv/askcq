...
|                       | - **GPT (26 CQs):** LLM-generated (GPT model)                                                                                           |
|                       | - **Gemini (42 CQs):** LLM-generated (Gemini model)                                                                                       |
...
| **Relevance** | LLM-assessed (Gemini) alignment with user story requirements on a 4-point Likert scale (explicit, inferable, contextual, or additional). Prompt engineering validated on a subset.        | Google Generative AI      |
...
| **Complexity ($c_1$)** | **Requirement Complexity:** Quantifies emerging ontological primitives (Concepts, Properties, Relationships, Filters, Cardinality, Aggregation). Extracted via LLM (Gemini) and scored. | Google Generative AI      |
...
-   `askcq/`
    ...
    -   `cq_generation.ipynb`: Notebook for LLM-based CQ generation from the user story.
    -   `overview.ipynb`: Notebook for expert analysis overview, CQ feature exploration, and general data analysis.
    -   `cq_readability.ipynb`: Notebook for computational readability analysis, including correlation analysis of different readability indices.
    -   `cq_relevance.ipynb`: Notebook for relevance scoring and rationale extraction using LLMs.
    -   `cq_complexity.ipynb`: Notebook for complexity feature extraction (requirement, linguistic, syntactic) and analysis from CQs.
    -   `cq_embeddings.ipynb`: Notebook for embedding-based semantic analysis of CQ sets.
...
-   `images/` (as per paper, e.g., `combined_measure_profile.pdf`)
-   `plots/`: Output figures and plots generated by the notebooks.
-   `tables/` (if generated by notebooks)
